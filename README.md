...
| 日期 | 版本 | 摘要 |
|---|---|---|
| 2026-04-14 | v0  | 員工 YAML schema 驗證 workflow 上線（CI 把關人事資料）|
| 2026-04-16 | v0.5 | schema 加 `projects` 欄位 + 全員員工檔同步 |
| 2026-04-18 | v1.0 🚀 | **MVP 上線**：16 個 perf label、sync 腳本、Dashboard 三頁、Auto-label workflow（GitHub Models / gpt-4o）全數開通 |
| 2026-04-27 | v5.1 | **三軸架構**取代 5 維度（AI 導入 / 如期 / 品質）；dashboard 重構、cron workflows、reopen 事件處理；AI Collab CI lint |
| 2026-04-27 | v5.2 | **客戶插單變更管理機制**：`scope-creep` 拆三級（external-handled / external-unmanaged / internal）+ `change-management-good` 加分機制 |
| 2026-04-27 | v5.3 | **需求變更機制 + PM 績效三層架構**：`requirement-change` / `rework-credit` / `spec-frozen` / `spec-not-frozen`；季度 PM Review issue template；dashboard 視覺化 6 項補強 |
| 2026-05-03 | docs | 新增 [`performance/HANDBOOK.md`](performance/HANDBOOK.md)（label 詳解、RD vs PM 維度差異、灰色地帶、申訴流程） |
